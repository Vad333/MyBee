...
class RS485Master:
    def __init__(self, uart_id=0, tx_pin=0, rx_pin=1, de_re_pin=2, baudrate=9600):
        """
        Инициализация Master-устройства для RS-485.
        :param uart_id: номер UART (0 или 1 для RP2040).
        :param tx_pin: пин для TX.
        :param rx_pin: пин для RX.
        :param de_re_pin: пин для управления DE/RE на MAX485.
        :param baudrate: скорость передачи.
        """
        # На некоторых платах (например, Nano RP2040) пины UART0 по умолчанию заняты USB, поэтому может потребоваться UART1
        ...
        self.de_re.value(0)  # Изначально в режиме приёма (RE=LOW)
    ...
    def send_packet(self, packet):
        ...
        # Ждём, пока буфер UART освободится (данные уйдут)
        while self.uart.any():
            time.sleep_ms(1)
        time.sleep_ms(10)  # Дополнительная небольшая задержка
...
if __name__ == "__main__":
    # Инициализация Master. Для Nano RP2040 может потребоваться uart_id=1, если UART0 занят USB
    master = RS485Master(uart_id=1, tx_pin=8, rx_pin=9, de_re_pin=2, baudrate=9600)
